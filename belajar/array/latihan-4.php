...
foreach ($array_id as $key => $value){
    foreach ($students[$key] as $key1 => $value1)echo $key1." = ".$value1."\n";
    echo "\n";
}
echo "\n\n\n";
asort($array_name);
...
foreach ($array_division as $key => $value){
    if ($value == "PHP Backend")foreach ($students[$key] as $key1 => $value1)echo $key1." = ".$value1."\n";
    echo "\n";
}
...
$usia_terendah = 0;
sort($array_age);
foreach ($students as $key)$jumlah_santri++;
